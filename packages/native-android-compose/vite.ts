...
const tryResolveServerOrigin = (server: ViteDevServer) => {
  ...
  if (!address || typeof address === 'string') {
    throw new Error(
      'Could not resolve a dev server origin for the nativeAndroidCompose environment.',
    )
  }
...
const waitForServerOrigin = async (
  ...
  while (Date.now() < deadline) {
    if (shouldCancel?.()) {
      throw new Error('Cancelled nativeAndroidCompose host startup.')
    }
    try {
  ...
  throw new Error('Could not resolve a dev server origin for the nativeAndroidCompose environment.')
}
      ...
      try {
        ...
        hostProcess.once('error', (error) => {
          environment.logger.error(
            `Failed to launch the nativeAndroidCompose host with "${command} ${args.join(' ')}": ${String(error)}`,
          )
        })
        hostProcess.once('exit', (code, signal) => {
          ...
          environment.logger.warn(
            `The nativeAndroidCompose host exited unexpectedly${code != null ? ` with code ${code}` : ''}${signal ? ` (${signal})` : ''}.`,
          )
        })
      } catch (error) {
        if (!closing) {
