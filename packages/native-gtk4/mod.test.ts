...
import { ApplicationWindow, Box, GTK4_DEFAULT_TAG_MAP, Text, TextField, Window } from './mod.ts'
...
describe('@eclipsa/native-gtk4', () => {
  ...
  it('keeps legacy component aliases wired to the same native tags', () => {
    expect(resolveNativeElementType(Window)).toBe('gtk4:window')
    expect(resolveNativeElementType(TextField)).toBe('gtk4:text-field')
  })
})
