...
const findModuleRecord = (key) => {
  const cachedRecord = moduleCache.get(key)
  if (cachedRecord) {
    return cachedRecord
  }
  for (const record of moduleCache.values()) {
    if (record.meta?.url === key || record.meta?.id === key || record.meta?.file === key) {
      return record
    }
  }
  return null
...
const createImportMeta = (moduleInfo, url) => {
  ...
  return {
    ...
    hot: globalState.__eclipsaNativeCreateHotContext?.(canonicalUrl),
    resolve(specifier) {
      return specifier
    },
    url: canonicalUrl,
